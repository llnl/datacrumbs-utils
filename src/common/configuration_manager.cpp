  ...
  if (argc >= 2) {
    ...
    if (first_arg == "--help" || first_arg == "-h") {
      DC_LOG_PRINT(
          ...
          "[--config_path <path>] [--user <user>] [--data_dir <path>] "
...
ConfigurationManager::ConfigurationManager(const std::filesystem::path& runtime_probe_file,
                                           bool print)
    : path(DATACRUMBS_INSTALL_CONFIGS_DIR),
      data_dir(DATACRUMBS_INSTALL_SHARED_DATA_DIR),
      name(runtime_probe_file.stem().string()),
      trace_log_dir(DATACRUMBS_LOG_DIR),
      capture_probes(),
      runtime_probes(),
...
void ConfigurationManager::derive_configurations() {
  ...
  std::string generated_file_suffix =
      this->user + "-" + this->run_id + "-" + hostname + "-" + this->name;
  ...
  std::string lookup_file_suffix = std::string(DATACRUMBS_INSTALL_USER) + "-" + this->name;
  ...
  std::string system_probe_file_name = "system-probe-" + lookup_file_suffix + ".json.gz";
...
void ConfigurationManager::load_runtime_system_configuration() {
  ...
  if (json_object_object_get_ex(root, "summary", &summary) &&
    ...
    const std::string config_path = json_string_or_empty(summary, "config_path");
    const std::string trace_dir = json_string_or_empty(summary, "trace_log_dir");
    const std::string config_name = json_string_or_empty(summary, "config_name");
    const std::string summary_user = json_string_or_empty(summary, "user");
    if (!config_path.empty()) path = config_path;
    if (!trace_dir.empty()) trace_log_dir = trace_dir;
    if (!config_name.empty()) name = config_name;
    if (!summary_user.empty() && std::getenv("DATACRUMBS_USER") == nullptr) user = summary_user;
  }
  ...
  if (json_object_object_get_ex(root, "system_configuration", &system_configuration) &&
    ...
    const std::string system_log_dir =
        json_string_or_empty(system_configuration, "DATACRUMBS_LOG_DIR");
    const std::string system_data_dir =
        json_string_or_empty(system_configuration, "DATACRUMBS_INSTALL_DATA_DIR");
    const std::string system_user = json_string_or_empty(system_configuration, "DATACRUMBS_USER");
    if (!system_log_dir.empty()) log_dir = system_log_dir;
    if (!system_data_dir.empty()) data_dir = system_data_dir;
    if (!system_user.empty() && std::getenv("DATACRUMBS_USER") == nullptr) user = system_user;
  }
